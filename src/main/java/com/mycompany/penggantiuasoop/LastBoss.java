...
public class LastBoss extends Enemy {
    ...
    public LastBoss(String name, int hp,int level,int gold, int physicalPower, int magicPower, int pDefense, int mgDefense) {
    ...
    actions.add(3); // Infernal Shockwave
    actions.add(4); // Wyrm's Wrath
    ...
    @Override
        ...
        System.out.println(name +" menggunakan skill Wyrm's Wrath");
            setPhysicalPower(getPhysicalPower() * 3/2);
            setPhysicalDefense(getPhysicalDefense()* 3/2);
            setMagicPower(getMagicPower()* 3/2);
            setMagicDefense(getMagicDefense()* 3/2);
